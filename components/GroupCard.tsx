...
const GroupCard: React.FC<GroupCardProps> = ({ group }) => {
  ...
  return (
    <div className="bg-white rounded-[3rem] p-10 flex flex-col h-full border border-slate-100 hover:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.08)] hover:-translate-y-3 transition-all duration-500 relative group overflow-hidden">
      {/* Subtle Background Badge */}
      <div className="absolute -top-10 -left-10 w-32 h-32 bg-green-50 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-700 blur-2xl"></div>
      
      <div className="flex justify-between items-start mb-8 relative z-10">
        <div className="bg-green-50 text-[#25D366] px-5 py-2 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] border border-green-100/50">
      ...
      <h3 className="text-2xl font-black text-slate-900 mb-5 line-clamp-1 urdu-font text-right leading-snug group-hover:text-[#25D366] transition-colors">
        {group.name}
      </h3>

      <div className="flex-grow text-right mb-10 relative z-10">
      ...
      <button
        onClick={handleJoin}
        className="w-full bg-slate-900 text-white py-5 rounded-[1.8rem] font-black text-sm transition-all flex items-center justify-center gap-4 shadow-xl shadow-slate-100 hover:bg-[#25D366] hover:shadow-green-200 active:scale-95 group-hover:scale-[1.02]"
      >
        <span>{t.joinNow}</span>
