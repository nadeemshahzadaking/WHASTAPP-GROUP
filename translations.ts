
import { Language, Translations, Category } from './types';

const categoriesEN: Record<Category, string> = {
  Pakistani: 'Pakistani', India: 'India', 'Hot Girls': 'Hot Girls', Videos: 'Videos', 'Horror Movies': 'Horror Clips',
  Education: 'Education', Jobs: 'Jobs', Business: 'Business', Islamic: 'Religious', 
  Entertainment: 'Entertainment', News: 'News', Sports: 'Sports', Tech: 'Technology', 
  Health: 'Health', Food: 'Food', Movies: 'Movies', Poetry: 'Poetry', 
  Shopping: 'Shopping', 'Real Estate': 'Property', Trading: 'Trading', 
  Freelancing: 'Freelancing', Programming: 'Programming', Gaming: 'Gaming', 
  Funny: 'Humor', Status: 'Status', Quotes: 'Quotes', Fashion: 'Fashion', 
  Travel: 'Travel', Automobiles: 'Vehicles', Agriculture: 'Agriculture', 
  Science: 'Science', History: 'History', Photography: 'Photography', 
  'Social Help': 'Help', Other: 'Others',
  Pro: 'Professional', Max: 'Maximum', 'Online Earning': 'Earning', 
  'IT & Software': 'IT', 'Web Development': 'Web Dev', 
  'Mobile Apps': 'Apps', 'AI & Technology': 'AI', 
  Students: 'Students', Scholarships: 'Scholarships', 'Exams & Preparation': 'Exams', 
  'Government Jobs': 'Govt Jobs', 'Private Jobs': 'Private Jobs', 
  'Overseas Jobs': 'Overseas', 'Visa & Immigration': 'Visa', 
  'Quran & Hadith': 'Quran', 'Islamic Education': 'Islamic Edu', 
  'Islamic Lectures': 'Lectures', Dawah: 'Dawah', 'Women Only': 'Women', 
  'Ladies Groups': 'Ladies', 'Marriage & Rishta': 'Matrimony', Family: 'Family', 
  Parenting: 'Parenting', Fitness: 'Fitness', Gym: 'Gym', Yoga: 'Yoga', 
  Medical: 'Medical', Doctors: 'Doctors', Nurses: 'Nurses', Dramas: 'Dramas', 
  Music: 'Music', Cricket: 'Cricket', Football: 'Football', PUBG: 'PUBG', 
  'Free Fire': 'Free Fire', 'E-commerce': 'E-commerce', Amazon: 'Amazon', 
  Daraz: 'Daraz', Forex: 'Forex', Crypto: 'Crypto', Property: 'Property', 
  Cars: 'Cars', Bikes: 'Bikes', Tourism: 'Tourism', Recipes: 'Recipes', 
  'Local Community': 'Local', 'City Groups': 'City', 'Breaking News': 'Breaking', 
  Memes: 'Memes', 'Study Abroad': 'Abroad', IELTS: 'IELTS', PTE: 'PTE', 
  'Freelance Marketplace': 'Market', Fiverr: 'Fiverr', Upwork: 'Upwork', 
  'Graphic Design': 'Design', 'Video Editing': 'Video Edit', 
  'Cyber Security': 'Security', 'Digital Marketing': 'Marketing', 
  'SEO Services': 'SEO', 'Stock Market': 'Stocks', 'Poetry & Ghazals': 'Poetry', 
  Motivational: 'Motivation', 'Art & Craft': 'Art', 'Pets & Animals': 'Pets'
};

const categoriesUR: Record<Category, string> = {
  Pakistani: 'پاکستانی', India: 'انڈیا', 'Hot Girls': 'ہاٹ گرلز', Videos: 'ویڈیو', 'Horror Movies': 'ہارر موویز کلپ',
  Education: 'تعلیم', Jobs: 'ملازمت', Business: 'کاروبار', Islamic: 'اسلامک', 
  Entertainment: 'تفریح', News: 'خبریں', Sports: 'کھیل', Tech: 'ٹیکنالوجی', 
  Health: 'صحت', Food: 'کھانے پینے', Movies: 'فلمیں', Poetry: 'شاعری', 
  Shopping: 'شاپنگ', 'Real Estate': 'پراپرٹی', Trading: 'ٹریڈنگ', 
  Freelancing: 'فری لانسنگ', Programming: 'پروگرامنگ', Gaming: 'گیمنگ', 
  Funny: 'لطیفے', Status: 'اسٹیٹس', Quotes: 'اقوال', Fashion: 'فیشن', 
  Travel: 'سفر و سیاحت', Automobiles: 'گاڑیاں', Agriculture: 'زراعت', 
  Science: 'سائنس', History: 'تاریخ', Photography: 'فوٹوگرافی', 
  'Social Help': 'سوشل ورک', Other: 'دیگر',
  Pro: 'پرو', Max: 'میکس', 'Online Earning': 'آن لائن کمائی', 
  'IT & Software': 'آئی ٹی', 'Web Development': 'ویب سائٹ', 
  'Mobile Apps': 'موبائل ایپس', 'AI & Technology': 'مصنوعی ذہانت', 
  Students: 'طلبا', Scholarships: 'اسکالرشپ', 'Exams & Preparation': 'امتحانات', 
  'Government Jobs': 'سرکاری نوکری', 'Private Jobs': 'پرائیویٹ نوکری', 
  'Overseas Jobs': 'بیرون ملک نوکری', 'Visa & Immigration': 'ویزہ', 
  'Quran & Hadith': 'قرآن و حدیث', 'Islamic Education': 'اسلامی تعلیم', 
  'Islamic Lectures': 'بیانات', Dawah: 'دعوت', 'Women Only': 'صرف خواتین', 
  'Ladies Groups': 'لیڈیز گروپس', 'Marriage & Rishta': 'رشتہ گروپس', Family: 'فیملی', 
  Parenting: 'والدین', Fitness: 'فٹنس', Gym: 'جم', Yoga: 'یوگا', 
  Medical: 'میڈیکل', Doctors: 'ڈاکٹرز', Nurses: 'نرسز', Dramas: 'ڈرامے', 
  Music: 'میوزک', Cricket: 'کرکٹ', Football: 'فٹبال', PUBG: 'پب جی', 
  'Free Fire': 'فری فائر', 'E-commerce': 'ای کامرس', Amazon: 'ایمیزون', 
  Daraz: 'دراز', Forex: 'فاریکس', Crypto: 'کرپٹو', Property: 'پراپرٹی', 
  Cars: 'گاڑیاں', Bikes: 'بائیکس', Tourism: 'سیاحت', Recipes: 'کھانوں کی ترکیب', 
  'Local Community': 'مقامی کمیونٹی', 'City Groups': 'شہری گروپس', 
  'Breaking News': 'بریکنگ نیوز', Memes: 'میمز', 'Study Abroad': 'بیرون ملک تعلیم', 
  IELTS: 'آئی لیٹس', PTE: 'پی ٹی ای', 
  'Freelance Marketplace': 'مارکیٹ پلیس', Fiverr: 'فائور', Upwork: 'اپ ورک', 
  'Graphic Design': 'گرافک ڈیزائن', 'Video Editing': 'ویڈیو ایڈیٹنگ', 
  'Cyber Security': 'سائبر سیکیورٹی', 'Digital Marketing': 'ڈیجیٹل مارکیٹنگ', 
  'SEO Services': 'ایس ای او', 'Stock Market': 'اسٹاک مارکیٹ', 'Poetry & Ghazals': 'اردو شاعری', 
  Motivational: 'موٹیویشنل', 'Art & Craft': 'آرٹ', 'Pets & Animals': 'پرندے و جانور'
};

export const TRANSLATIONS: Record<Language, Translations> = {
  en: {
    title: 'WHATSAPP HUB 2026',
    subtitle: 'Search & Join the best WhatsApp communities instantly.',
    searchPlaceholder: 'Search name, category or description...',
    searchBtn: 'Search',
    addBtn: 'Add Group',
    viewGroupsBtn: 'View All Groups',
    homeLink: 'Home',
    viewLink: 'Browse',
    addLink: 'Submit',
    trendingLink: 'Trending',
    aboutLink: 'About',
    officialLink: 'Official',
    // Added values for en
    aboutTitle: 'About Us',
    aboutContent: 'WhatsApp Hub 2026 is the premier destination for discovering and joining active WhatsApp communities from around the world. Our platform provides a safe and organized directory of thousands of groups across various categories.',
    officialTitle: 'Official Channels',
    officialSub: 'Connect with our verified social media handles and official groups.',
    safetyTitle: 'Safety Rules',
    safetyMessage: 'Please follow community guidelines.\nDo not share spam or illegal content.\nRespect all group members.',
    closeBtn: 'Close',
    backBtn: 'Back',
    clicksLabel: 'Views',
    linkError: 'Link already exists in our database',
    joinNow: 'JOIN NOW',
    addNewGroup: 'Add New Group',
    groupName: 'Group Name',
    groupLink: 'WhatsApp Link',
    selectCategory: 'Select Category',
    descriptionLabel: 'Description',
    submitBtn: 'SUBMIT NOW',
    successTitle: 'Success!',
    successMsg: 'Group added successfully!',
    footerLine1: '© 2026 WhatsApp Hub. Professional Directory.',
    footerLine2: 'Not affiliated with WhatsApp Inc.',
    groupsListTitle: 'Discover Groups',
    groupsFound: 'communities found',
    noGroups: 'No results found',
    noGroupsSub: 'Try different keywords.',
    allCategories: 'All Categories',
    categories: categoriesEN,
    dir: 'ltr'
  },
  ur: {
    title: 'واٹس ایپ ہب 2026',
    subtitle: 'بہترین واٹس ایپ کمیونٹیز تلاش کریں اور ابھی جوائن کریں۔',
    searchPlaceholder: 'نام، کیٹیگری یا تفصیل تلاش کریں...',
    searchBtn: 'تلاش کریں',
    addBtn: 'گروپ ڈالیں',
    viewGroupsBtn: 'تمام گروپس دیکھیں',
    homeLink: 'ہوم',
    viewLink: 'ایکسپلور',
    addLink: 'شامل کریں',
    trendingLink: 'ٹرینڈنگ',
    aboutLink: 'ہمارے بارے میں',
    officialLink: 'آفیشل لنکس',
    // Added values for ur
    aboutTitle: 'ہمارے بارے میں',
    aboutContent: 'واٹس ایپ ہب 2026 دنیا بھر سے فعال واٹس ایپ کمیونٹیز کو دریافت کرنے اور ان میں شامل ہونے کے لیے ایک بہترین پلیٹ فارم ہے۔ ہمارا پلیٹ فارم مختلف زمروں میں ہزاروں گروپس کی ایک محفوظ اور منظم ڈائریکٹری فراہم کرتا ہے۔',
    officialTitle: 'آفیشل چینلز',
    officialSub: 'ہمارے تصدیق شدہ سوشل میڈیا ہینڈلز اور آفیشل گروپس سے جڑیں۔',
    safetyTitle: 'حفاظتی اصول',
    safetyMessage: 'براہ کرم کمیونٹی گائیڈ لائنز پر عمل کریں۔\nسپیم یا غیر قانونی مواد شیئر نہ کریں۔\nگروپ کے تمام ممبران کا احترام کریں۔',
    closeBtn: 'بند کریں',
    backBtn: 'واپس',
    clicksLabel: 'مشاہدات',
    linkError: 'یہ لنک پہلے سے موجود ہے',
    joinNow: 'جوائن کریں',
    addNewGroup: 'نیا گروپ شامل کریں',
    groupName: 'گروپ کا نام',
    groupLink: 'واٹس ایپ لنک',
    selectCategory: 'کیٹیگری منتخب کریں',
    descriptionLabel: 'تفصیل',
    submitBtn: 'ابھی جمع کریں',
    successTitle: 'کامیابی!',
    successMsg: 'گروپ کامیابی سے شامل ہو گیا!',
    footerLine1: '© 2026 واٹس ایپ ہب۔ پروفیشنل ڈائریکٹری۔',
    footerLine2: 'واٹس ایپ سے کوئی تعلق نہیں ہے۔',
    groupsListTitle: 'گروپس دریافت کریں',
    groupsFound: 'گروپس ملے',
    noGroups: 'کچھ نہیں ملا',
    noGroupsSub: 'کوئی اور لفظ لکھ کر دیکھیں۔',
    allCategories: 'تمام کیٹیگریز',
    categories: categoriesUR,
    dir: 'rtl'
  },
  ro: {
    title: 'WA-HUB 2026',
    subtitle: 'Search and join WhatsApp groups.',
    searchPlaceholder: 'Search...',
    searchBtn: 'Search',
    addBtn: 'Add',
    viewGroupsBtn: 'View All',
    homeLink: 'Home',
    viewLink: 'Browse',
    addLink: 'Add',
    trendingLink: 'Trending',
    aboutLink: 'About',
    officialLink: 'Official',
    // Added values for ro
    aboutTitle: 'Despre Noi',
    aboutContent: 'WhatsApp Hub 2026 este destinația principală pentru a descoperi și a te alătura comunităților WhatsApp active din întreaga lume. Platforma noastră oferă un director sigur și organizat de mii de grupuri din diverse categorii.',
    officialTitle: 'Canale Oficiale',
    officialSub: 'Conectați-vă cu conturile noastre oficiale de social media și grupurile oficiale.',
    safetyTitle: 'Reguli de Siguranță',
    safetyMessage: 'Vă rugăm să respectați regulile comunității.\nNu partajați spam sau conținut ilegal.\nRespectați toți membrii grupului.',
    closeBtn: 'Închide',
    backBtn: 'Back',
    clicksLabel: 'Views',
    linkError: 'Link exists',
    joinNow: 'JOIN',
    addNewGroup: 'Add Group',
    groupName: 'Name',
    groupLink: 'Link',
    selectCategory: 'Category',
    descriptionLabel: 'Details',
    submitBtn: 'SUBMIT',
    successTitle: 'Success!',
    successMsg: 'Added!',
    footerLine1: '© 2026 WhatsApp Hub.',
    footerLine2: 'Not affiliated.',
    groupsListTitle: 'Groups',
    groupsFound: 'found',
    noGroups: 'Nothing',
    noGroupsSub: 'Try again.',
    allCategories: 'All',
    categories: categoriesEN,
    dir: 'ltr'
  }
};
