  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // اب یہ ایرر کی وجہ بھی بتائے گا
        const errorMessage = data.error || (t.dir === 'rtl' ? 'ڈیٹا محفوظ کرنے میں غلطی آئی۔ دوبارہ کوشش کریں۔' : 'Error saving data. Please try again.');
        alert(errorMessage + (data.details ? `\n\nDetails: ${data.details}` : ''));
      }
    } catch (error: any) {
      console.error('Submission error:', error);
      alert(t.dir === 'rtl' ? 'انٹرنیٹ یا کنکشن کا مسئلہ ہے۔' : `Network error: ${error.message}`);
    } finally {
