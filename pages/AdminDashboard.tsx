...
const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'groups' | 'settings'>('groups');
  ...
  const handleUpdatePassword = async () => {
    if (newPassword.length < 4) {
      alert('پاس ورڈ کم از کم 4 ہندسوں کا ہونا چاہیے');
      return;
    }
    ...
    try {
      ...
      if (error) throw error;
      alert('پاس ورڈ کامیابی سے تبدیل ہو گیا!');
      setNewPassword('');
    } catch (err) {
      alert('پاس ورڈ اپ ڈیٹ نہیں ہو سکا');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row">
      <aside className="w-full md:w-72 bg-slate-900 text-white p-8 flex flex-col gap-10">
        ...
        <nav className="flex-grow space-y-4">
          <button 
            ...
            className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-bold text-sm transition-all ${activeTab === 'groups' ? 'bg-white/10' : 'hover:bg-white/5 text-slate-400'}`}
          >
            <span>📊</span> ڈیش بورڈ
          </button>
          <button 
            onClick={() => setActiveTab('settings')}
            className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-bold text-sm transition-all ${activeTab === 'settings' ? 'bg-white/10' : 'hover:bg-white/5 text-slate-400'}`}
          >
            <span>⚙️</span> سیٹنگز
          </button>
      ...
      <main className="flex-grow p-6 md:p-12 overflow-x-hidden">
        {activeTab === 'groups' ? (
          <>
            <header className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6 text-right">
              <div dir="rtl">
          ...
          <div className="max-w-xl mx-auto text-right" dir="rtl">
            <h1 className="text-3xl font-black text-slate-900 mb-8 urdu-font">اکاؤنٹ سیٹنگز</h1>
            <div className="bg-white p-10 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-8">
              <div>
                <label className="block text-xs font-black text-slate-400 uppercase mb-3 mr-1">نیا پاس ورڈ درج کریں</label>
                <input 
                  type="text"
                  value={newPassword}
                  onChange={(e) => setNewPassword(e.target.value)}
                  className="w-full px-6 py-4 rounded-2xl border-2 border-slate-50 focus:border-slate-900 outline-none font-bold text-center"
              ...
              <button 
                ...
                className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-lg hover:bg-black transition-all active:scale-95 disabled:opacity-50"
              >
                {updateLoading ? 'اپ ڈیٹ ہو رہا ہے...' : 'پاس ورڈ تبدیل کریں'}
              </button>
            </div>
          </div>
        )}
